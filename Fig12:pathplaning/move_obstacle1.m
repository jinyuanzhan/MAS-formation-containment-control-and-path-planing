function [obst_pos, obstacle_struct] = move_obstacle1(obst_pos, num_obstacles, obstacle_struct, ~)
    k = 4; % 指定只移动第五个障碍物
    if k <= num_obstacles % 确保障碍物索引不超出范围
        % 更新边框的位置
        obstacle_struct.o(k).XData = obstacle_struct.o(k).XData - 0.1;
        obstacle_struct.o(k).YData = obstacle_struct.o(k).YData + 0.7;
        ...
        x_vertices = get(fill_handle, 'XData') - 0.1;
        y_vertices = get(fill_handle, 'YData') +0.7;
